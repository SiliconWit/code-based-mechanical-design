...
A chip rated for 125 degrees C junction temperature will throttle, glitch, or fail permanently if the heat sink cannot dissipate its power budget. Picking a heat sink from a catalog is a gamble unless you understand the full thermal resistance chain from junction to ambient, and the right fin geometry depends on airflow, orientation, and available space in ways that are not obvious from a datasheet alone. In this lesson you will model that resistance chain from first principles, compare straight-fin, pin-fin, and radial fin geometries in CadQuery, and sweep parameters with Python to build thermal performance maps that show exactly which configuration keeps your chip cool. #ThermalDesign #HeatSink #CadQuery

## Learning Objectives

By the end of this lesson, you will be able to:

1. Model the thermal resistance chain from junction to ambient ($R_{jc}$, $R_{cs}$, $R_{sa}$)
2. Calculate natural convection heat transfer using Nusselt and Rayleigh number correlations
3. Compare straight rectangular, pin-fin, and radial/sunflower fin geometries
4. Implement each fin type as parametric CadQuery models
5. Sweep fin count, height, and spacing to generate thermal performance maps with matplotlib
6. Optimize heat sink geometry to minimize thermal resistance under manufacturing constraints
7. Export the optimized heat sink as STEP and STL with full thermal justification
...
```text
  Thermal Resistance Chain (Junction to Ambient)
  ──────────────────────────────────────────
  Tj (junction)
    │
    ├── Rjc (junction to case)     0.5-3.0 C/W
    │   Package type, die attach
    │
    ├── Rcs (case to sink)         0.1-2.0 C/W
    │   Thermal paste or pad
    │
    ├── Rsa (sink to ambient)      ← We design this
  ...
  Analogy: series electrical circuit
  Q (watts) = I (amps)
  dT (degrees) = V (volts)
  R_thermal = R_electrical
```
...
:::tip[Instrument it]
The model predicts a junction temperature; the only way to trust it is to measure the real one. Put an NTC thermistor or a digital sensor (a precise TMP117, or a DS18B20) on the component case and another on the heat-sink base to read the actual thermal resistance, or a thermocouple with a MAX31855 for anything hot. An [ESP32 or STM32](/education/sensor-actuator-interfacing-stm32/) reads them, and can do more than verify: drive a PWM fan or [throttle the load](https://siliconwit.io) when the temperature climbs, so the part never cooks.
:::

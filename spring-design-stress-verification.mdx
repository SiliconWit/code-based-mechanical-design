...
:::tip[Instrument it]
A spring's calculated rate and its real rate often differ, because wire diameter, free length, and end conditions all drift in manufacturing, and a 3D-printed or wound spring relaxes over time. Press it against a load cell while reading travel with a dial gauge or a linear potentiometer, and you get the true force-deflection curve, and so the actual rate. An ESP32 or STM32 logs it, and on a spring that holds a preload you can [watch for relaxation](https://siliconwit.io) as the force quietly sags over thousands of cycles.
:::

...
Before writing any CAD code, you need three tools: **CadQuery** (the geometry kernel), **Jupyter** (the interactive notebook), and **OCP CAD Viewer** (the 3D visualizer for VS Code).

<Steps>
1. **Create a virtual environment** (recommended to avoid package conflicts):
   ```bash
   python -m venv cadquery-env
   source cadquery-env/bin/activate  # Linux/macOS
   # cadquery-env\Scripts\activate   # Windows
   ```

2. **Install CadQuery and supporting tools**:
   ```bash
   pip install --upgrade pip
   pip install cadquery ocp-cad-viewer jupyter
   ```
...
4. **Start Jupyter and verify the installation**:
   ```bash
   jupyter notebook
   ```
   ...
   # Quick verification: a simple box
   box = cq.Workplane("XY").box(10, 10, 10)
   cq.exporters.export(box, "test_box.step")
   print("CadQuery is working! Exported test_box.step")
   ```
   If you are using VS Code with the OCP CAD Viewer extension, you can also visualize interactively:
   ```python
   from ocp_vscode import show
   show(box)
   ```

5. **Alternative: use CQ-editor** if you prefer a standalone GUI:
   ```bash
   pip install cq-editor
   ```
   CQ-editor includes a built-in 3D viewer and code editor, but this course assumes the Jupyter + OCP workflow.
</Steps>

:::tip[Conda Alternative]
If you use Anaconda/Miniconda, CadQuery is also available via conda-forge:
```bash
conda install -c conda-forge cadquery
```
This can resolve OpenCASCADE build issues on some systems.
:::

...
Before writing any CAD code, you need three tools: **CadQuery** (the geometry kernel), **Jupyter** (the interactive notebook), and **OCP CAD Viewer** (the 3D visualizer for VS Code). CadQuery requires **Python 3.9 or newer** (3.10+ recommended). Check your version with `python3 --version` before proceeding.
...
2. **Install CadQuery, the viewer, and Jupyter**:
   ```bash
   pip install --upgrade pip
   pip install cadquery ocp-vscode jupyter
   ```

3. **Install the OCP CAD Viewer extension in VS Code**:
   Open VS Code, go to Extensions (Ctrl+Shift+X), search for "OCP CAD Viewer," and install it. This gives you a live 3D viewport that updates as you run CadQuery scripts.
...
4. **Verify the installation**:
   ...
   print("CadQuery is working! Exported test_box.step")
   ```
   If you have VS Code with OCP CAD Viewer, you can also visualize interactively:
   ```python
...
3. **Install CadQuery, the viewer, and Jupyter**:
   ```bash
   pip install --upgrade pip
   pip install cadquery ocp-vscode jupyter
   ```

4. **Install the OCP CAD Viewer extension in VS Code**:
   Open VS Code, go to Extensions (Ctrl+Shift+X), search for "OCP CAD Viewer," and install it.
...
3. **Install Jupyter and the VS Code viewer**:
   ```bash
   pip install ocp-vscode jupyter
   ```

4. **Verify the installation** (same as the pip tab):
   ```python
   import cadquery as cq
   box = cq.Workplane("XY").box(10, 10, 10)
   cq.exporters.export(box, "test_box.step")
   print("CadQuery is working!")
   ```
</Steps>
...
def make_hex_bolt(size, length, thread_type="cosmetic"):
    ...
    tip_cut = (
        ...
        .cone(d / 2.0, 0, chamfer_height, centered=(True, True, False))
